...
**手動デプロイ（緊急時のみ）:**
```bash
ssh -i ~/Downloads/mieru-clinic-key.pem root@210.131.223.161
...
### Windows PowerShell
```powershell
ssh -i $env:USERPROFILE\Downloads\mieru-clinic-key.pem root@210.131.223.161
```
...
### 設定ファイル
```
/etc/nginx/sites-available/mieru-clinic
```
...
### 手動バックアップ
```bash
sudo -u postgres pg_dump dental_check > /var/backups/mieru-clinic/dental_$(date +%Y%m%d_%H%M%S).sql
```

### リストア
```bash
sudo -u postgres psql -d dental_check < /var/backups/mieru-clinic/dental_YYYYMMDD_HHMMSS.sql
```
...
|------|-----|
| 実行時刻 | 毎日 午前3時 |
| 保存場所 | `/var/backups/mieru-clinic/` |
| 保持期間 | 30日間（古いものは自動削除） |
| ログ | `/var/log/dental-backup.log` |

### バックアップ確認
```bash
ls -la /var/backups/mieru-clinic/
```
...
### バックアップスクリプト
```
/var/www/mieru-clinic/scripts/backup-db.sh
```
...
### プロジェクトパス
```
C:\Users\hacha\Documents\mieru-clinic
```

### 起動方法（コマンドプロンプト）
```cmd
cd C:\Users\hacha\Documents\mieru-clinic
npm run dev -- -p 3002
```
...
- 本番ブランチ: `main`
- リポジトリ: `https://github.com/KotaroTao/MIERU-CLINIC`
